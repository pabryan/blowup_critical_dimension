...
Imposing the initial condition \(\psi(0) = 0\) gives a one-parameter family of solutions, parametrised by \(\psi'(0)\).
...
\]
which is not integrable. Therefore we conclude that \(C_1 = 0\).
...
\[
G'(z) = 2F_{\beta}(z) \Rightarrow G(z) = z^2(1 + \bigo(z^{\gamma}))
\]
and so
\[
\sqrt{\abs{G(\zeta)}} = z \sqrt{1 + \bigo(z^{\gamma})} = z + \bigo(z^{\gamma/2})
\]
is continuous (even differentiable) near \(z = 0\).
...
\textbf{The question now is whether this all makes sense! Effectively, we need \(H\) to be defined for \(z \in (0, a)\) mapping \((0, a) \to \R\) with \(H(0) = -\infty\) and \(H'(0) = \infty\). This seems likely to be okay since presumably \(H\) increases up from \(-\infty\) at least for a while. Maybe we can be very explicit as follows:}

\[
G(z) = 2 \int_0^z F_{\beta}(u) du
\]
is the solution of \(G(0) = 0, G'(z) = 2 F_{\beta}(z)\) and is \(C^{2,\alpha}\).

Now we have then \(G'(0) = 2 F_{\beta}(0) = 0\), and \(G''(0) = 2 F_{\beta}'(0) = 2\). So \(G > 0\) for a neighbourhood of \(z=0\) but not necessarily globaly. Ideally it would be globally positive.  So if we keep integrating past a zero \(F\), we may end up having \(G\) becoming negative. This will stop our solution being a classical solution but maybe it's okay in the generalised sense since \(\sqrt{\abs{G(z)}}\) is \(C^{1/2}\) or perhaps better if \(F\) does not meet the \(z\)-axis tangentially. So we just need to ask that if \(F(z_0) = 0\), then \(F'(z_0) \ne 0\) and \(F\) being \(C^{1,\alpha}\) gives what we need. Then we get \(\sqrt{G}\) is \(C^{1,\alpha/2}\).

Just pretending that \(G \geq 0\) for the moment anyway we could then take any \(a_0 \in (0, a)\) with \(b_0 > a_0\)) and define
\[
H(z) = \int_{a_0}^z \frac{1}{\sqrt{\abs{G(u)}}} du
\]
which satisfies \(H'(z) = \frac{1}{\sqrt{\abs{G(u)}}}\) and
\[
H(0) = - \lim_{z\to 0} \int_0^{a_0} \frac{1}{\sqrt{\abs{G(u)}}} du
\]

Now I just need that this integral is \(\infty\). This should follow from the asymptotics of \(F\) and \(G\) I think. Near \(z = 0\) we have \(F = z + \bigo(z^{1 + \alpha})\) and \(G = z^2 + \bigo(z^{2 + \alpha})\) in which case \(\sqrt{\abs{G}} = z + \bigo(z^{1 + \alpha/2})\) so that \(1/\sqrt{\abs{G}}\) will integrate like \(1/z\) which goes to \(\infty\) as required.

Then for the range that \(G > 0\), \(H\) is invertible since it has strictly positive derivative and we may define
\[
\xi(r) = H^{-1} (\ln r^{\beta} + \lambda).
\]

Actually, we can always take \(a_0 \in (0, a)\) near \(0\) so that \(G > 0\) on \((0, b_0)\). Moreover, the integrand defining \(H\) is non-negative. The only thing that could go wrong, stopping us from integrating all the way to \(a\) is if \(G \to 0\) so that \(\sqrt{\abs{G}} \to 0\) is not integrable for some \(b_0 < a_0\). If that doesn't happen we can integrate all the way to \(a\) and we expect the integral to be \(\infty\) as we approach \(a\) just as it is as we approach \(0\). Then we get \(H : (0, a) \to (-\infty, \infty)\) an increasing, hence invertible function.

\textbf{Look at the behaviour of \(\xi\) as \(\lambda \to \pm \infty\) and \(\lambda \to 0\). This will be used for blow up.}
