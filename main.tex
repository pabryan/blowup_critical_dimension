...
Any such solution \(\xi\) extends uniquely to \(r \in [0, 1]\) by choosing any \(\rho_2 \in (0, \rho_1)\) and solving the uniformly elliptic problem \eqref{eq:stationary} subject to the boundary conditions \(\psi(\rho_2) = \xi(\rho_2)\), \(\psi(1) = \psi_1\) on \([\rho_1, 1]\). Recall \(\psi_1\) is any real number occurring as the boundary data at \(r=1\) in \eqref{eq:pde}. Imposing the initial condition \(\psi(0) = 0\) gives a one-parameter family of solutions, parametrised by \(\psi'(0)\). Note that the Cartheodary theorem doesn't guarantee uniqueness but for our purposes, existence is sufficient.
...
\xi(r, t) = \xi_0(r^{\beta})
\]
for some function \(\xi_0\) with \(\xi_0(0) = 0\) and some real number \(\beta > 0\). We may assume that \(\xi_0'(0) = 1\) by replacing \(\xi_0\) with \((\xi_0')^{-1} \xi_0\).

\textbf{we'll need to know \(\xi_0' \ne 0\)! The regularity of the solution (dependent on the regularity of \(F\)) will be very important here. If a smooth enough solution can be found we may write it as  \(c_k y^k + c_{k+1} y^{k+1} + \cdots\) with \(c_k \ne 0\). Then we would take \(\psi_0 = c_k y + c_{2k} y + \cdots\) and \(\beta=k\). Of course we would need to show that \(c_{k+1} = \cdots = c_{2k -1} = 0\) which we hope will follow from asymptotics.}

Then \(\partial_t \xi = 0\) and
\[
\begin{split}
...
\end{cases}
\]
The equation for \(\xi_0\) is effectively the same equation we started with, but the form given here is useful however for analysing the role of \(\beta\) in determining blow up. It also allows us to prove existence of stationary solutions In the notation of our heuristics we have
\[
A = \frac{F'(0)}{\beta^2}.
\]
...
Thus we must solve
\[
\frac{\zeta'}{\sqrt{G(\zeta) + C_1}} = 1
\]
and hence our solution satisfies
\[
H_{C_1} (\zeta(w)) = w, \quad \text{where} \quad H_{C_1} (0) = -\infty \quad \text{and} \quad H_{C_1}'(Z) = \frac{1}{\sqrt{G(Z) + C_1}}.
\]
In terms of \(\xi\),
\[
H_{C_1} (\xi(r, t)) = H_{C_1} (\xi_0(r^{\beta})) = \ln r^{\beta},
\]
and if \(H_{C_1}\) is invertible,
\[
\xi(r, t) = H_{C_1}^{-1} (\ln r^{\beta}).
\]
