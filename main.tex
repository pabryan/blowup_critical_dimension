...
\subsection{Conditions for blow up}
...
\begin{enumerate}
\item If there exists a sub-solution \(\xi\) to equation \eqref{eq:pde} such that \(\xi(r, t=0) \leq \psi(r, t=0)\) and \(\xi'(0, t) \to \infty\) as \(t \to T\), then \(\psi'(0, t) \to \infty\) as \(t \to T\). Likewise, if \(\psi(r, t = 0) \leq \xi(r, t=0)\) for a super-solution with \(\xi'(0, t) \to -\infty\) as \(t \to T\), then \(\psi'(0, t) \to \infty\) as \(t \to T\).
\item If there exists a super-solution \(\xi\) to equation \eqref{eq:pde} such that \(-\xi(r, t=0) \leq \psi(r, t=0) \leq \xi(r, t-0)\) with \(\abs{\xi'(0, t)} \leq M < \infty\) for \(t \in [0, T)\) and some \(M > 0\) then there exists an \(N\) with \(0 < N < \infty\) such that \(\|\psi(\cdot, t)\|_{C^1} < N\) for all \(t \in [0, T)\).
\end{enumerate}
\end{lemma}
...
The argument in this section is based on constructing barriers from stationary solutions that either force or prevent blow up depending on the roots of the characteristic equation.

\subsection{The case \(\|\psi\|_{\infty} < a\)}
