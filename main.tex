...
For the degree \(\ell\)-harmonic map heat flow, \(A = \ell^2\) while for the Yang-Mills flow, \(A = 4\). Thus for Yang-Mills and higher degree harmonic map heat flow (\(\ell > 1\)), we have \(\beta_0 > 1\) and no \(C^1\) blow up occurs. However for, \(\ell \leq 4\) and Yang-Mills we do expect \(C^2\) blow up. For \(\ell > 4\), there is no \(C^2\) blow up.
...
\]
By adjusting \(C_1\) if necessary, we may assume that \(G(0) = 0\). Then solutions with finite \(\zeta'(-\infty)\) and satisfying the initial condition \(\zeta(-\infty) = 0\) must satisfy
\[
C_1 = 2 \zeta(-\infty) \zeta'(-\infty) - G(0) = 0.
\]
Thus we must solve
\[
...
\[
H (\zeta(w)) = w + \lambda, \quad \text{where} \quad H (0) = -\infty \quad \text{and} \quad H'(Z) = \frac{1}{\sqrt{G(Z)}}
\]
...
\end{pmatrix}.
\]
The right hand side is a continuous function of \((u,v)\) (in fact Lipschitz since \(F\) is Lipschitz) for each fixed \(r > 0\) and is continuous in \(r > 0\) for each fixed \((u, v)\) but only measureable for \(r \in [0, 1]\). The Cartheodary theorem guarantees the existence of an absolutely continuous solution \((u, v)\) on \([0, \rho_1)\) for some  \(\rho_1 > 0\) given any initial condition \((u_0, v_0)\) at \(r=0\). But for any \(\rho_2 \in (0, \rho_1)\), the right hand side is uniformly Lipschitz in \((u, v)\) hence the Picard-Lindel\"of theorem gives a unique \(C^2\) solution on \([\rho_2, 1]\) given the data \((u(\rho_2), v(\rho_2))\) which must also agree on \((0, \rho_1]\) with the solution obtained from the Cartheodary theorem. Since this is true for any \(\rho_2\), given any initial data \((\psi(0), \psi'(0))\), there exists a \(C^2\) solution on \((0, 1)\).

For our purposes, we take \(\psi(0) = 0\) and so we obtain a one-parameter family of solutions, parametrised by \(\psi'(0)\).
