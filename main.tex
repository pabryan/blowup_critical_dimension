...
Combining equations \eqref{eq:Hinv_first_order_exp} and \eqref{eq:w_asymptotics} we find that for \(w \in (-\infty, M_{\epsilon})\) where \(M_{\epsilon} = \min\{H(\delta(\epsilon)), H(\eta)\})\), we have
\begin{equation}
\label{eq:Hinv_first_order_exp_estimate}
\abs{H^{-1}(w) - e^w} \leq A (e^w)^{(1+\alpha)/(1+\epsilon)}.
\end{equation}
This is true, in particular for all \(\epsilon \in (0, \alpha)\). Setting
\[
\bar{\alpha} = \frac{\alpha - \epsilon}{1 + \epsilon}
\]
we have \(\bar{\alpha}\) ranges over all values in \((0, \alpha)\) as \(\epsilon\) ranges over \((0, \alpha)\) and \(1 + \bar{\alpha} = \tfrac{1+\alpha}{1+\epsilon}\).

To finish, let \(w = \ln \lambda r\) in equation \eqref{eq:Hinv_first_order_exp_estimate} to obtain that for \(r \in (0, e^{M_{\epsilon}}/\lambda)\),
\[
\begin{split}
