...
\end{split}
\end{equation}
for some \(A > 0\) and \(z \in [0, \epsilon)\) for some \(\epsilon > 0\), or equivalently for \(w \in (-\infty, M)\) for some \(M\). Now we iterate (noting that \(H^{-1}(w) > 0\)),
\[
\begin{split}
\abs{H^{-1}(w) - e^w} &\leq A(H^{-1}(w))^{1+\alpha} \leq A (e^w + \abs{H^{-1}(w)-e^w})^{1+\alpha} \leq A(e^w + A(H^{-1}(w))^{1+\alpha})^{1+\alpha} \\
&\leq A\left[e^w + A\left(e^w + A(H^{-1}(w))^{1+\alpha}\right)^{1+\alpha}\right]^{1+\alpha}
\end{split}
\]
To ease notation, let us write for each fixed \(w\),
\[
h = h_{e^w, 1+\alpha, A} (v) = A(e^w + A v^{1+\alpha})^{1+\alpha}
\]
so that we may restate the inequality as
\[
\begin{split}
\abs{H^{-1}(w) - e^w} &\leq h(H^{-1}(w)) \\
&\leq h(h(H^{-1}(w))) \\
&\leq h^{\circ k} (H^{-1}(w))
\end{split}
\]
with the latter inequality following by induction and where \(h^{\circ k} = \underbrace{h \circ \cdots \circ h}_{k \text{ times}}\). Therefore,
\[
\abs{H^{-1}(w) - e^w} \leq \limsup_{k\to\infty} h^{\circ k} (H^{-1}(w))
\]
for all \(w \in (-\infty, W(\epsilon))\).

{\color{red}Now what we want is:} for any \(u,v > 0\) (or even just sufficiently small perhaps) and \(p > 0\),
\[
\limsup_{k\to\infty} h_{u, p, A}^{\circ k} (v) \leq B u^p
\]
where \(h_{u, p, A} = A(u + A v^p)^p\). For then we obtain
\[
\abs{H^{-1}(w) - e^w} \leq \limsup_{k\to\infty} h_{e^w, 1+\alpha, A} ^{\circ k} (H^{-1}(w)) \leq B e^{(1+\alpha) w}.
\]
With \(w = \ln \lambda r\) we obtain that given any \(\epsilon > 0\), provided \(r \in [0, e^{W(\epsilon)}/\lambda)\), we have
\[
\begin{split}
\abs{\xi(r) - \lambda r} &= \abs{H^{-1} (\ln \lambda r) - e^{\ln \lambda r}} \leq B e^{(1+\alpha) \ln \lambda r} = B\lambda^{1+\alpha} r^{1+\alpha}
\end{split}
\]
as required.
\end{proof}
