...
The method of proof as has been used by many authors \cite{MR2332425,MR1180392} proceeds by suitable construction of sub and super solutions either forcing or preventing blow up. These are obtained from one-parameter families of stationary solutions \(\xi = \xi_{\lambda}\) satisfying \(L(\xi) = -\tfrac{1}{r^2} \bar{F} (\xi)\) and \(\xi(0) = 0\). Since \(L\) is second order, this leaves the free parameter \(\lambda\) specifying \(\xi'(0)\). By allowing this parameter to depend on \(t\) in just the right way, we obtain the desired sub and super solutions, \(\xi_{\lambda(t)} (r)\).
...
\begin{pmatrix}
v \\
\frac{1}{r^2} \bar{F}(u) - \frac{C}{r} v - \frac{F'(0)}{r} u
\end{pmatrix}.
\]
...
Let us first consider the case \(C = 1\) which is satisfied in critical dimension. It will be convenient to work with \(F\) for the moment rather than the linearisation. We can solve equation \eqref{eq:stationary} explicitly (in terms of \(F\)) by changing variables, \(y = e^w\) and letting \(\zeta(w) = \xi(e^w)\), whence our problem becomes
\[
\begin{cases}
...
\]
Notice that near \(z = 0\) we have \(F (z) = z + \bigo(z^{1+\gamma})\) so that
\[
G'(z) = 2F(z) \Rightarrow G(z) = z^2(1 + \bigo(z^{2 + \gamma}))
\]
and so
\[
...
\]
This is also why we cannot define \(H(z) = \int_{0}^z \frac{1}{u(s)} ds\).
...
\[
\begin{split}
\xi' &= \frac{1}{r}  \sqrt{G(\xi(r))} = \frac{1}{r}  \sqrt{G \circ H^{-1}(\ln r + \lambda)} \\
&= \frac{1}{r} \left[H^{-1} (\ln r + \lambda)  + \bigo(H^{-1} (\ln r + \lambda))^{1 + \gamma}\right] \\
&= 
\end{split}
\]
...
\[
L_+ = \partial_r - \frac{\beta^+ - 1}{r}, \quad L_- = \partial_r - \frac{\beta^-}{r}
\]
giving
\[
L = L_+ \circ L_-.
\]
This follows since \(\beta^{\pm}\) are the roots of the characteristic equation,
\[
...
\]
and since in the composition \(L+ \circ L_-\), there is an additional term from \(L_+\) acting on the \(1/r\) coefficient in \(L_-\).


\begin{rem}
Alternatively, we could define
\[
L_+ = \partial_r - \frac{\beta^+}{r}, \quad L_- = \partial_r - \frac{\beta^- - 1}{r}
\]
in which case,
\[
L = L_- \circ L_+.
\]
It makes no difference which we choose, and so we prefer the first choice, \(L = L_+ \circ L_-\).
\end{rem}

Then our stationary solution satisfies
\[
L_+ \circ L_- \xi = \frac{1}{r^2} F(\xi)
\]
so that
\[
\xi = L_-^{-1} \circ L_+^{-1} (\frac{1}{r^2} F(\xi)).
\]
Note that \(L_{\pm}\) being first order, linear differential operators have non-unique left inverses, determined up to arbitrary constants. The condition \(\xi(0) = 0\) determines one constant, while the other is determined by \(\psi'(0)\) which provides us with the free parameter \(\lambda = \psi'(0)\).

\textbf{Write it down explicitly and determine how the free parameters occur}.

\textbf{Work out asymptotics and free parameter dependence which is very important for the construction of barriers}.
