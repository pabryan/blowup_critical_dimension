...
\]
Recalling that we require \(\xi(0) = 0\), and that \(\beta_- < 0\), we set \(C_- = 0\) and thus seek solutions of
\begin{equation}
...
\xi = (\Id - G)^{-1} \left(C_+ r^{\beta_+}\right).
\end{equation}
...
In order prove the existence and uniqueness of a solution to \eqref{eq:stationaryC_implicit} we use a suitable contraction mapping principle on a suitable function space to ensure the existence of a unique fixed point. To this end, recall that there is a \(z_0 \in (0, a)\) and an \(M > 0\) such that for all \(z \in [0, z_0]\),
\[
\abs{\bar{F}(z)} \leq M z^{1+\alpha}.
\]
Given \(\epsilon > 0\) and \(r_0 < \left(\tfrac{z_0}{\lambda + \epsilon}\right)^{1/(\beta_+)}\), define
\begin{equation}
\label{eq:contraction_space}
X = X_{\epsilon,r_0} = \left\{\xi \in C([0, 1] \to \R_{\geq 0}) : \forall r \in [0, r_0], \abs{\xi(r)} \leq \epsilon r^{\beta_+} \right\}.
\end{equation}
We claim that
\begin{enumerate}
\item \(X \subseteq C([0, 1])\) is closed with respect to the sup-norm, \(\|\cdot\|_{\infty}\), \label{itm:closed}
\item \(G : X \to X\), \label{itm:self_map}
\item \(G\) is a strict contraction mapping with respect to the sup-norm, \(\|\cdot\|_{\infty}\). \label{itm:contraction}
\end{enumerate}
Assuming the claim, then there exists a unique fixed point of \(G\) in \(X\). Existence and uniqueness of solutions to \eqref{eq:stationaryC_implicit} is obtained by taking any \(\lambda > 0\) and defining
\[
X_{\lambda} = \{\xi : \xi - \lambda r^{\beta_+} \in X\}, \quad T_{\lambda} : \xi \in X_{\lambda} \mapsto G(\xi) + \lambda r^{\beta_+} \in X_{\lambda}.
\]
Then \(X_{\lambda}\) also has a unique fixed point, which is the desired solution.

Thus we must verify the claim. Point \ref{itm:closed} is straight forward: for each \(z \in \R\) and \(r \in [0, r_0]\), the function
\[
\Phi : (z, r) \mapsto \abs{z} - \epsilon r^{\beta_+}
\]
is a continuous map \(\R \times [0, r_0] \to \R\). Now if \((\xi_n)_{n \in \N} \subseteq X\) is such that \(\xi_n \to \xi\) with respect to \(\|\cdot \|_{\infty}\), then for every \(r \in [0, r_0]\) we have \(\xi_n(r) \to \xi(r)\) and \(\Phi(\xi_n(r), r) \leq 0\). Thus by continuity, for every \(r \in [0, r_0]\), \(\Phi(\xi(r), r) \leq 0\) and \(\xi \in X\).

For point \ref{itm:self_map}, we need to show that for \(\xi \in X\) and all \(r \in [0, r_0]\) we have
\[
\abs{G(\xi(r))} \leq \epsilon r^{\beta_+}.
\]
Being more explicit in the bounds of integration in the definition of \(G\) from \eqref{eq:stationaryC_G}, we take
\[
\begin{split}
G(\xi(r)) = r^{\beta_-} \int_0^r \left[\rho^{\beta_+ - \beta_- - 1} \int_0^{\rho} \left(\tau^{-(\beta_+ + 1)} \bar{F}(\xi(\tau)) \right)d\tau\right] d\rho.
\end{split}
\]
To show that \(G\) is well defined and maps to \(X\), note that in defining \(X\) in \eqref{eq:contraction_space}, we used that there is a \(z_0 \in (0, a)\) and an \(M > 0\) such that for all \(z \in [0, z_0]\),
\[
\abs{\bar{F}(z)} \leq M z^{1+\alpha}
\]
and that we chose \(r_0 < \left(\tfrac{z_0}{\lambda + \epsilon}\right)^{1/(\beta_+)}\). Thus from the definition of \(X\), for \(r \in [0, r_0]\) we have
\[
0 \leq \xi(r) \leq (\lambda + \epsilon) r^{\beta_+} \leq (\lambda + \epsilon) r_0^{\beta_+} < z_0,
\]
and
\[
\abs{\bar{F}(\xi(r))} \leq M \abs{\xi(r)}^{1+\alpha} \leq M (\lambda + \epsilon)^{1+\alpha} r^{(1+\alpha)\beta_+}.
\]


Given \(\lambda, \epsilon > 0\) and \(r_0 < \left(\tfrac{z_0}{\lambda + \epsilon}\right)^{1/(\beta_+)}\), define
\begin{equation}
\label{eq:contraction_space}
...
We claim that
\begin{enumerate}
\item \(X \subseteq C([0, 1])\) is a closed with respect to the sup-norm, \(\|\cdot\|_{\infty}\), \label{itm:closed}
...
Point \ref{itm:closed} is straight forward: for each \(r \in [0, r_0]\), the function
\[
...
Now in defining \(X\) in \eqref{eq:contraction_space}, we used that there is a \(z_0 \in (0, a)\) and an \(M > 0\) such that for all \(z \in [0, z_0]\),
\[
\abs{\bar{F}(z)} = M z^{1+\alpha}
\]
and that we chose \(r_0 < \left(\tfrac{z_0}{\lambda + \epsilon}\right)^{1/(\beta_+)}\). Thus from the definition of \(X\), for \(r \in [0, r_0]\) we have
\[
0 \leq \xi(r) \leq (\lambda + \epsilon) r^{\beta_+} \leq (\lambda + \epsilon) r_0^{\beta_+} < z_0,
...
\]
Letting \(w_0 = - \frac{1}{1-C}\), we have the range of \(w\) and initial condition at \(r=0\):
\[
\begin{cases}
w \geq w_0, \quad \zeta(w_0) = 0; & C < 1 \\
w \leq w_0, \quad \zeta(-\infty) = 0; & C > 1.
\end{cases}
\]

This change of variables leads to \(\zeta\) satisfying
\begin{equation}
...
Multiplying both sides by \(\zeta'\) and rearranging, we may rewrite this as
\[
...
\end{cases}
\]
. Letting \(q(w) = \frac{1}{2} [1 + (1-C)w]^2\), we make a further rewrite as
\[
m(w)^2 [(\zeta')^2]' =  F(\zeta) \zeta'.
\]

{\color{red}Surely this is solvable! We now have the awkward factor \([1 + (1-C)w]^{-2}\) preventing us from directly integrating \(F\) as we did for the \(C = 1\) case. The difficulty with applying a general existence theorem is that the function \(m(w)^{-2}\) is not integrable all the way to \(w_0 = -\tfrac{1}{1-C}\). This might actually be what we want - in the \(C = 1\) case we get something similar where we can't integrate \(H\) all the way to \(r = 0\) but it all worked out anyway and we got the \(\ln r\) term.

  If take our initial condition at \(w_1 > w_0\), everything is indeed fine for \(w \geq w_1\), but we want to extend the solution back to \(w_0\) where it should satisfy \(\zeta(w_0) = 0\). Because \(F\) is Lipschitz, we do in fact get the right hand side of our ODE is Lipschitz for \(w > w_0\), but it's not uniformly Lipschitz. So we do get a unique solution taking any initial condition at \(w_1 > w_0\).

  Perhaps we can solve the equation with initial condition \(\xi(w_0 + \epsilon) = f(\epsilon)\) with \(f\) chosen appropriately so that \(f(\epsilon) \to 0\) as \(\epsilon \to 0\) to get \(\xi_{\epsilon}\). Then take \(\xi = \lim_{\epsilon \to 0} \xi_{\epsilon}\). The trick is we'll need some a-priori estimates and recall that the Lipschizt constant is not uniform. In fact it's something like
  \[
  m(w)^{-2} \text{Lip}(F).
  \]
Maybe the structure of the function \(m(w)\) allows us to get the desired estimates? It's just a quadratic...
}.
