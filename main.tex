...
\]
Recall by equations \eqref{eq:G} and \eqref{eq:dzeta} in the proof of \Cref{lem:stationaryC1} we have
\[
...
By \Cref{thm:comparison}, \(\eta(r\mu(t)) \leq \psi(r, t)\). But \(\xi(r, t) = \eta(r\mu(t))\) and \(\mu\) blows up as \(t \to T\) and hence so too must \(\psi\).
\end{proof}
